...
export default function AutoSaveControl({ onSave, isSaving, onIntervalChange }: AutoSaveControlProps) {
    ...
    return (
        <div className="relative flex items-center shadow-lg shadow-black/10 rounded-full" ref={dropdownRef}>
            ...
            <button
                onClick={onSave}
                disabled={isSaving}
                className="px-4 py-2 text-sm font-medium text-white bg-black dark:bg-white dark:text-black hover:bg-gray-800 dark:hover:bg-gray-200 transition-all active:scale-95 disabled:opacity-50 rounded-l-full border-r border-gray-700 dark:border-gray-200"
            >
            ...
            <button
                ...
                className="px-2 py-2 text-white bg-black dark:bg-white dark:text-black hover:bg-gray-800 dark:hover:bg-gray-200 rounded-r-full transition-colors focus:outline-none"
            ...
            {isOpen && (
                <div className="absolute top-full right-0 mt-2 w-48 bg-white dark:bg-zinc-800 rounded-lg shadow-xl border border-zinc-200 dark:border-zinc-700 py-1 z-50 overflow-hidden animate-in fade-in zoom-in-95 duration-100">
                    <div className="px-4 py-2 text-xs font-semibold text-zinc-500 dark:text-zinc-400 border-b border-zinc-100 dark:border-zinc-700/50">
                        Auto-save Interval
                    </div>
